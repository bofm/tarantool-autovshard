-- src: https://github.com/moonlibs/tarantoolapp/blob/d97e1226e9d448ad2e2f3dfd9ec2f18f0dead662/templates/basic/template/t/tnt/init.lua
...
local function compare_versions(expected, version)
    ...
    local function reducer(res, l, r)
        ...
        if tonumber(l) == tonumber(r) then return nil end
    ...
    local function check_version(expected, version)
        ...
        if type(version) == 'string' then version = split_version(version) end
...
return {
    ...
    cleanup = function() return cleanup end,

    logfile = function() return fio.pathjoin(dir, 'tarantool.log') end,

    log = function()
        local fh = fio.open(fio.pathjoin(dir, 'tarantool.log'), 'O_RDONLY')
        if fh == nil then box.error(box.error.PROC_LUA, errno.strerror()) end
