-- src: https://github.com/kyleconroy/lua-state-machine
local machine = {}
...
local function call_handler(handler, params) if handler then return handler(unpack(params)) end end
...
local function create_transition(name)
    ...
    local function transition(self, ...)
        if self.asyncState == NONE then
            ...
            if beforeReturn == false or leaveReturn == false then return false end

            self.asyncState = name .. "WaitingOnLeave"

            if leaveReturn ~= ASYNC then transition(self, ...) end
        ...
        elseif self.asyncState == name .. "WaitingOnLeave" then
            ...
            if enterReturn ~= ASYNC then transition(self, ...) end
        ...
        elseif self.asyncState == name .. "WaitingOnEnter" then
            ...
            if string.find(self.asyncState, "WaitingOnLeave") or
                string.find(self.asyncState, "WaitingOnEnter") then
                self.asyncState = NONE
                transition(self, ...)
                return true
            end
...
local function add_to_map(map, event)
    if type(event.from) == 'string' then
        ...
        for _, from in ipairs(event.from) do map[from] = event.to end
    end
end
...
function machine:is(state) return self.current == state end
...
function machine:cannot(e) return not self:can(e) end
...
function machine:todot(filename)
    ...
    for _, event in pairs(self.options.events) do
        if type(event.from) == 'table' then
            for _, from in ipairs(event.from) do transition(event.name, from, event.to) end
        else
